...
`OpenRouterLLMClient` calls OpenRouter live when a key is present. Both return raw
claim dicts `{text, source_sentence, source_page?}`; claims.py validates them.
"""
...
def get_default_client(offline: bool = True) -> LLMClient:
    """Offline or no key -> deterministic mock. Online -> OpenRouter."""
    if not offline and os.environ.get("OPENROUTER_API_KEY"):
        try:
