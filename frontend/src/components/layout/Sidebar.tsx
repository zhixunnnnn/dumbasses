...
type NavItem = {
  key: Extract<RouteName, "dashboard" | "assistant">;
  label: string;
...
const STATIC_LINKS = [
  ...
  { label: "Watchlists", icon: <BookMarked size={17} /> },
    ...
    <aside className="hidden w-60 shrink-0 flex-col border-r border-hairline bg-surface px-3 py-5 md:flex">
      ...
      <nav className="mt-6 flex flex-col gap-0.5">
        ...
        {STATIC_LINKS.map((item) => (
          <button
            key={item.label}
            className="flex items-center gap-2.5 rounded-md px-2 py-1.5 text-sm text-muted transition hover:bg-raised/60 hover:text-txt"
          >
