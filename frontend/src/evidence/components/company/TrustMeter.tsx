...
export default function TrustMeter({ raters }: { raters: Raters }) {
  const vals = RATERS.map((r) => ({ ...r, v: raters[r.key] as number | null })).filter((r) => r.v !== null);
      ...
      <div className="relative mt-1 h-9">
        ...
        {vals.map((r) => (
          <div key={r.label} className="group absolute -translate-x-1/2" style={{ left: `${r.v}%`, top: 0 }}>
            ...
            <div className="h-2.5 w-2.5 rounded-full ring-2 ring-surface" style={{ backgroundColor: r.color }}>
